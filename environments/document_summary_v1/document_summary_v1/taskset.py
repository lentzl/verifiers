...
MARKDOWN_OWNER_RECOVERY_FEEDBACK = (
    ...
    "launched. Track received receipts separately from handles; preserve receipts "
    "already stored. Validate each arriving message against its named job and store "
    "its matching chapter_id and summary_path in the receipts dictionary. If some "
    "receipts are still missing, end this assistant turn with a brief waiting message "
    "and NO tool call. This yields for child messages; it does not declare the task "
    "complete. Do not use an empty IPython call, sleep, join or await on a handle to wait. "
...
MARKDOWN_CHILD_RECOVERY_FEEDBACK = (
    ...
    "complete summary in one pathlib.Path.write_text operation. Its integer return "
    "counts characters written, not words; it is not an error or a word-limit verdict. After a successful "
    ...
    for chapter in document["chapters"]:
        ...
        jobs[worker] = {
            ...
            "prompt": (
                ...
                "Path.write_text returns the number of characters written, not the word count. "
                "A successful numeric return is not a reason to rewrite the same text. "
        ...
        if self.config.mode == "owner_direct":
            ...
            data = DocumentSummaryData(
                ...
                prompt=(
                    ...
                    "Initialize a separate receipts dictionary once and preserve it across messages. "
                    "After spawning, end this assistant turn with a brief waiting message and "
                    "NO tool call. This yields for child messages, not a claim that the document "
                    "is complete. Do not poll files or agents, issue empty IPython calls, or "
                    "join or await a retained handle. "
                    "Do not wait inside an IPython sleep loop: retained handles do not disappear "
                    "when a child finishes, and a running cell prevents you from handling its message. "
                    "Each child will write its own summary and send a JSON receipt containing "
                    "chapter_id and summary_path. On each arrival, validate the sender and both "
                    "fields against its index job, then store the receipt keyed by worker name "
                    "without resetting earlier receipts. If receipts are missing, end the turn "
                    "again with ordinary assistant text and no tool call. When all named children "
                    "have matching stored receipts, read their assigned summary files. Assemble those texts "
                ...
                system_prompt=(
                    "You are working on document summarization inside Prime Agent. Your recursive "
                    "depth and current assignment determine your role: at depth zero, own delegation "
                    "and assembly; as a chapter child, author only your assigned summary, report its "
                    "saved path to the parent and stop. Use the persistent IPython kernel for file I/O. "
                    "Only the owner spawns native rlm children and assembles their unchanged text. "
